...
module.exports = {
    ...
    module: {
        rules: [     //内部配置loader
           ...
           它是vue文件的一个加载器，将template/script/style转换成js模块。
    ...
    //开发服务器    安装webpack-dev-server后自动使用
    devServer: {
        // port: 8081, //指定服务器内存
        open: true, // 自动打开浏览器
        quiet: true, // 不做太多日志输出
    ...
    //配置开启source-map调试,运行的是打包后的文件,开启这个可以在错误的时候提示错误在源文件的哪一行
    ...
    resolve:{
        ...
        /*  'vue$': 'vue/dist/vue.esm.js',  //表示精准匹配:去找node_modules下vue/dist/vue.esm.js文件 
           为什么要找vue/dist/vue.esm.js? 因为vue默认的版本不带编译器,需要手动修改,找到vue带编译器的版本文件
           例如
            components : {注册组件}
